...
router.get("/auth/shopify/callback", wrap(authMiddleware), requireAdmin, wrap(shopifyOAuthCallback));
...
router.post("/sync/product-images/stop", requireAdmin, wrap(stopProductImagesSyncHandler));
router.post("/sync/orders", requireAdmin, wrap(syncOrdersHandler));
...
router.post("/backfill/orders/stop", requireAdmin, wrap(stopOrdersBackfillHandler));
router.post("/sync/inventory-adjustments", requireAdmin, wrap(syncInventoryAdjustmentsHandler));
