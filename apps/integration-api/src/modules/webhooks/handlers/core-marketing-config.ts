...
import { getOrCreatePixelKey, rotatePixelKey } from "../../../../../../src/marketing/db/marketing.repository";
...
function resolveShopDomainValue(queryValue: unknown, bodyValue: unknown) {
  const fromQuery = normalizeShopDomain(queryValue);
  if (fromQuery) return fromQuery;
  return normalizeShopDomain(bodyValue);
}
...
export async function marketingWebhooksStatusHandler(req: Request, res: Response) {
  try {
    const shopDomain = resolveShopDomainValue(req.query.shopDomain, req.body?.shopDomain);
    if (!shopDomain) {
      res.status(400).json({ error: "shopDomain requerido" });
      return;
    }
